...
public class JDBCDelete {
	 ...
	 */
	public JDBCDelete(String table, String where, String test) {
		...
		if (resultTable.size() > 0) {
			...
			Query = "DELETE FROM " + table + " * WHERE id = " + resultTable.get(0); 
			try {
				deleteRecordFromDbUserTable();
			} catch (SQLException e) {
				System.out.println(e.getMessage());
			...
			JOptionPane.showMessageDialog(null, "The table and conditional you entered may"
					+ " not have been entered correctly.", "SELECT ERROR", 
					JOptionPane.ERROR_MESSAGE);
		}
	}
